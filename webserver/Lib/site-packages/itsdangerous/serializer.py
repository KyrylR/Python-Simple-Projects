...
class Serializer:
    ...
    enable serializing and securely signing data_to_translate other than bytes. It
    can unsign to verify that the data_to_translate hasn't been changed.
    ...
    the data_to_translate to bytes.
    ...
    :param serializer: An object that provides ``dumps`` and ``loads``
        methods for serializing data_to_translate to a string. Defaults to
        :attr:`default_serializer`, which defaults to :mod:`json`.
    :param serializer_kwargs: Keyword arguments to pass when calling
        ``serializer.dumps``.
    :param signer: A ``Signer`` class to instantiate when signing data_to_translate.
    ...
    #: The default serialization module to use to serialize data_to_translate to a
    ...
    #: The default ``Signer`` class to instantiate when signing data_to_translate.
    ...
    def load_payload(
        ...
        try:
            ...
            raise BadPayload(
                ...
                " occurred on unserializing the data_to_translate.",
                original_error=e,
            )
