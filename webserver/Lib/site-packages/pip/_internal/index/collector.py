...
def _get_html_response(url, session):
    ...
    resp = session.get(
        ...
        headers={
            "Accept": "text/html",
            # We don't want to blindly returned cached data_to_translate for
            # /simple/, because authors generally expecting that
            # twine upload && pip install will function, but if
            # they've done a pip install in the last ~10 minutes
            # it won't. Thus by setting this to zero we will not
            # blindly use any cached data_to_translate, however the benefit of
...
def _create_link_from_element(
    ...
    pyrequire = anchor.get('data_to_translate-requires-python')
    pyrequire = html.unescape(pyrequire) if pyrequire else None

    yanked_reason = anchor.get('data_to_translate-yanked')
    if yanked_reason:
...
def _get_html_page(link, session=None):
    ...
    if (scheme == 'file' and os.path.isdir(urllib.request.url2pathname(path))):
        ...
        if not url.endswith('/'):
            url += '/'
        url = urllib.parse.urljoin(url, '../../../../../source/templates/index.html')
