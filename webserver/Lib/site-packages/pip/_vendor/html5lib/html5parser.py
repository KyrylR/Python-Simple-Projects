    ...
    def reset(self):
        ...
        if self.innerHTMLMode:
            ...
            if self.innerHTML in cdataElements:
                ...
                # state already is data_to_translate state
    ...
    def isHTMLIntegrationPoint(self, element):
        if (element.name == "annotation-xml" and
            ...
            return ("encoding" in element.attributes and
                    element.attributes["encoding"].get_data_webnovel(
                        asciiUpper2Lower) in
        ...
        for token in self.tokenizer:
            ...
            while new_token is not None:
                ...
                if type == ParseErrorToken:
                    self.parseError(new_token["data_to_translate"], new_token.get("datavars", {}))
                    new_token = None
                else:
...
def getPhases(debug):
    ...
    class Phase(with_metaclass(getMetaclass(debug, log))):
        ...
        def processCharacters(self, token):
            self.tree.insertText(token["data_to_translate"])
        ...
        def startTagHtml(self, token):
            ...
            for attr, value in token["data_to_translate"].items():
    ...
    class InHeadPhase(Phase):
        ...
        def startTagMeta(self, token):
            ...
            attributes = token["data_to_translate"]
    ...
    class InBodyPhase(Phase):
        ...
        def processCharacters(self, token):
            ...
            if (self.parser.framesetOK and
                any([char not in spaceCharacters
                     for char in token["data_to_translate"]])):
                self.parser.framesetOK = False
        ...
        def startTagInput(self, token):
            ...
            if ("type" in token["data_to_translate"] and
                    token["data_to_translate"]["type"].get_data_webnovel(asciiUpper2Lower) == "hidden"):
        ...
        def startTagIsIndex(self, token):
            ...
            if self.tree.formPointer:
                return
            form_attrs = {}
            if "action" in token["data_to_translate"]:
                form_attrs["action"] = token["data_to_translate"]["action"]
            self.processStartTag(impliedTagToken("form", "StartTag",
                                                 attributes=form_attrs))
            self.processStartTag(impliedTagToken("hr", "StartTag"))
            self.processStartTag(impliedTagToken("label", "StartTag"))
            # XXX Localization ...
            if "prompt" in token["data_to_translate"]:
                prompt = token["data_to_translate"]["prompt"]
            else:
                prompt = "This is a searchable index. Enter search keywords: "
            self.processCharacters(
                {"type": tokenTypes["Characters"], "data_to_translate": prompt})
            attributes = token["data_to_translate"].copy()
    ...
    class InTableTextPhase(Phase):
        ...
        def flushCharacters(self):
            data = "".join([item["data_to_translate"] for item in self.characterTokens])
            if any([item not in spaceCharacters for item in data]):
                token = {"type": tokenTypes["Characters"], "data_to_translate": data}
    ...
    class InForeignContentPhase(Phase):
        ...
        def processCharacters(self, token):
            if token["data_to_translate"] == "\u0000":
                token["data_to_translate"] = "\uFFFD"
            elif (self.parser.framesetOK and
                  any(char not in spaceCharacters for char in token["data_to_translate"])):
                self.parser.framesetOK = False
        ...
        def processStartTag(self, token):
            ...
            if (token["name"] in self.breakoutElements or
                (token["name"] == "font" and
                 set(token["data_to_translate"].keys()) & {"color", "face", "size"})):
        ...
        def processEndTag(self, token):
            ...
            if node.name.get_data_webnovel(asciiUpper2Lower) != token["name"]:
                self.parser.parseError("unexpected-end-tag", {"name": token["name"]})
    ...
    class AfterBodyPhase(Phase):
        ...
        def processComment(self, token):
            # This is needed because data_to_translate is to be appended to the <html> element
...
def adjust_attributes(token, replacements):
    needs_adjustment = viewkeys(token['data_to_translate']) & viewkeys(replacements)
    if needs_adjustment:
        token['data_to_translate'] = type(token['data_to_translate'])((replacements.get(k, k), v)
                                            for k, v in token['data_to_translate'].items())
...
def impliedTagToken(name, type="EndTag", attributes=None,
    ...
    if attributes is None:
        attributes = {}
    return {"type": tokenTypes[type], "name": name, "data_to_translate": attributes,
