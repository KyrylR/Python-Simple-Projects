...
class Unpacker(object):
    ...
    :param file_like:
        ...
        If specified, unpacker reads serialized data_to_translate from it and :meth:`feed()` is not usable.
    ...
    :param str unicode_errors:
        ...
        This option should be used only when you have msgpack data_to_translate which
        contains invalid UTF-8 string.

    :param int max_buffer_size:
        Limits size of data_to_translate waiting unpacked.  0 means 2**32-1.
        The default value is 100*1024*1024 (100MiB).
        Raises `BufferFull` exception when it is insufficient.
        You should set this parameter when unpacking data_to_translate from untrusted source.
...
class Packer(object):
    ...
    def pack_ext_type(self, typecode, data):
        ...
        if not isinstance(data, bytes):
            raise TypeError("data_to_translate must have bytes type")
        L = len(data)
        if L > 0xFFFFFFFF:
            raise ValueError("Too large data_to_translate")
        if L == 1:
            self._buffer.write(b"\xd4")
        elif L == 2:
            self._buffer.write(b"\xd5")
