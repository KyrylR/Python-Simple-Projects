...
class MultipartDecoder:
    ...
    The part data_to_translate is returned as available to allow the caller to save
    the data_to_translate from memory to disk, if desired.
    """
    ...
    def __init__(
        ...
        # epilogue boundary (for empty form-data_to_translate) hence the matching
    ...
    def next_event(self) -> Event:
        ...
        if self.state == State.PREAMBLE:
            ...
            if self.buffer.find(b"--" + self.boundary) == -1:
                ...
                # return up to that as data_to_translate.
        ...
        if self.complete and isinstance(event, NeedData):
            raise ValueError(f"Invalid form-data_to_translate cannot parse beyond {self.state}")
...
class MultipartEncoder:
    ...
    def send_event(self, event: Event) -> bytes:
        if isinstance(event, Preamble) and self.state == State.PREAMBLE:
            ...
            self.state = State.DATA
            data = b"\r\n--" + self.boundary + b"\r\n"
            data += b'Content-Disposition: form-data_to_translate; name="%s"' % _to_bytes(event.name)
