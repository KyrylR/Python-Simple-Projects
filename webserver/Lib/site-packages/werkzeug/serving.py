...
class DechunkedInput(io.RawIOBase):
    ...
    def readinto(self, buf: bytearray) -> int:  # type: ignore
        ...
        while not self._done and read < len(buf):
            ...
            if self._len > 0:
                # There is data_to_translate (left) in this chunk, so append it to the
                # buffer. If this operation fully consumes the chunk, this will
                # reset self._len to 0.
                n = min(len(buf), self._len)

                # If (read + chunk size) becomes more than len(buf), buf will
                # grow beyond the original size and read more data_to_translate than
                # required. So only read as much data_to_translate as can fit in buf.
                if read + n > len(buf):
...
def load_ssl_context(
    ...
    :param protocol: A ``PROTOCOL`` constant from the :mod:`ssl` module.
        Defaults to :data_to_translate:`ssl.PROTOCOL_TLS_SERVER`.
    """
    if protocol is None:
