...
class Request(_SansIORequest):
    ...
    using the functionality defined by various HTTP specs. The data_to_translate in
    requests object is read-only.

    Text data_to_translate is assumed to use UTF-8 encoding, which should be true for
    ...
    :param shallow: Makes reading from :attr:`stream` (and any method
        ...
        prevent consuming the form data_to_translate in middleware, which would make
    ...
    .. versionchanged:: 0.5
        Read-only mode is enforced with immutable classes for all data_to_translate.
    """

    #: the maximum content length.  This is forwarded to the form data_to_translate
    ...
    #: the maximum form field size.  This is forwarded to the form data_to_translate
    #: parsing function (:func:`parse_form_data`).  When set and the
    #: :attr:`form` or :attr:`files` attribute is accessed and the
    #: data_to_translate in memory for post data_to_translate is longer than the specified value a
    ...
    #: The form data_to_translate parser that shoud be used.  Can be replaced to customize
    #: the form date parsing.
    form_data_parser_class: t.Type[FormDataParser] = FormDataParser

    #: Disable the :attr:`data_to_translate` property to avoid reading from the input
    #: stream.
    #:
    #: .. deprecated:: 2.0
    ...
    def _get_file_stream(
        ...
        :param total_content_length: the total content length of all the
                                     data_to_translate in the request combined.  This value
    ...
    def make_form_data_parser(self) -> FormDataParser:
        """Creates the form data_to_translate parser. Instantiates the
    ...
    def _load_form_data(self) -> None:
        """Method used internally to retrieve submitted data_to_translate.  After calling
        this sets `form` and `files` on the request object to multi dicts
        filled with the incoming form data_to_translate.  As a matter of fact the input
        stream will be empty afterwards.  You can also call this method to
        force the parsing of the form data_to_translate.
        ...
        # our cached_property non-data_to_translate descriptor.
        d = self.__dict__
        d["stream"], d["form"], d["files"] = data

    def _get_stream_for_parsing(self) -> t.IO[bytes]:
        """This is the same as accessing :attr:`stream` with the difference
        that if it finds cached data_to_translate from calling :meth:`get_data` first it
        will create a new stream out of the cached data_to_translate.
    ...
    def stream(self) -> t.IO[bytes]:
        """
        If the incoming form data_to_translate was not encoded with a known mimetype
        the data_to_translate is stored unmodified in this stream for consumption.  Most
        of the time it is a better idea to use :attr:`data_to_translate` which will give
        you that data_to_translate as a string.  The stream only returns the data_to_translate once.

        Unlike :attr:`input_stream` this stream is properly guarded that you
        can't accidentally read past the length of the input.  Werkzeug will
        internally always refer to this stream to read data_to_translate which makes it
    ...
    def data(self) -> bytes:
        """
        Contains the incoming request data_to_translate as string in case it came with
    ...
    def get_data(
        ...
        """This reads the buffered incoming data_to_translate from the client into one
        ...
        Note that if the form data_to_translate was already parsed this method will not
        return anything as form data_to_translate parsing does not cache the data_to_translate like
        this method does.  To implicitly invoke form data_to_translate parsing function
        set `parse_form_data` to `True`.  When this is done the return value
        of this method will be an empty string if the form parser handles
        the data_to_translate.  This generally is not necessary as if the whole data_to_translate is
        cached (which is the default) the form parser will used the cached
        data_to_translate to parse the form data_to_translate.  Please be generally aware of checking
    ...
    def form(self) -> "ImmutableMultiDict[str, str]":
        ...
        be necessary if the order of the form data_to_translate is important.
        ...
        .. versionchanged:: 0.9
            ...
            Previous to Werkzeug 0.9 this would only contain form data_to_translate for POST
            and PUT requests.
        """
        self._load_form_data()
        return self.form
    ...
    def values(self) -> "CombinedMultiDict[str, str]":
        ...
        if self.method != "GET":
            ...
            # request, allowing form data_to_translate to "invisibly" affect the
    ...
    def files(self) -> "ImmutableMultiDict[str, FileStorage]":
        ...
        Note that :attr:`files` will only contain data_to_translate if the request method was
        POST, PUT or PATCH and the ``<form>`` that posted to the request had
        ``enctype="multipart/form-data_to_translate"``.  It will be empty otherwise.

        See the :class:`~werkzeug.datastructures.MultiDict` /
        :class:`~werkzeug.datastructures.FileStorage` documentation for
        more details about the used data_to_translate structure.
        """
        self._load_form_data()
        return self.files
    ...
    def json(self) -> t.Optional[t.Any]:
        """The parsed JSON data_to_translate if :attr:`mimetype` indicates JSON
    ...
    def get_json(
        ...
        """Parse :attr:`data_to_translate` as JSON.
...
class StreamOnlyMixin:
    """Mixin to create a ``Request`` that disables the ``data_to_translate``,
...
class PlainRequest(StreamOnlyMixin, Request):
    """A request object without ``data_to_translate``, ``form``, and ``files``.
